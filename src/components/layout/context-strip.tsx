...
function panelClassForTone(tone: StripTone) {
	...
	if (tone === "danger") {
		return "border-[color:color-mix(in_oklab,var(--status-danger),transparent_72%)] bg-[linear-gradient(155deg,color-mix(in_oklab,var(--status-danger-bg),white_16%),color-mix(in_oklab,var(--card),transparent_0%))]";
	}
...
export function ContextStrip({ items, className }: { items: ContextStripItem[]; className?: string }) {
	return (
			...
			{items.map(item => {
				...
				return (
					<article
						key={item.label}
						className={cn(
							"ds-panel-muted ds-grid-overlay relative overflow-hidden rounded-2xl p-4 transition duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:-translate-y-[1px] hover:shadow-[var(--shadow-soft)]",
						...
						<div className="mt-3">
							<div className="font-display text-[clamp(1.35rem,3vw,1.9rem)] font-semibold leading-[0.98] tracking-[-0.05em] text-foreground">{item.value}</div>
