...
function appendNativeNodes(
  children: React.ReactNode,
  groups: NativeGroup[],
  activeGroupLabel?: React.ReactNode,
): boolean {
  let valid = true;
...
const selectTriggerBaseClass = cn(
  controlBaseClass,
  "justify-between gap-3 pr-10 text-left shadow-[var(--shadow-inner)] data-[placeholder]:text-muted-foreground/90 [&>span]:line-clamp-1",
);
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      className={cn(
        "relative z-50 max-h-80 min-w-[8rem] overflow-hidden rounded-[1.1rem] border border-[color:color-mix(in_oklab,var(--foreground),transparent_88%)] bg-[linear-gradient(160deg,color-mix(in_oklab,var(--card),white_10%),color-mix(in_oklab,var(--card),transparent_2%)_56%,color-mix(in_oklab,var(--accent),transparent_94%))] text-foreground shadow-[var(--shadow-lift)] backdrop-blur-xl",
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ...
    className={cn("px-2.5 py-1.5 font-subheader text-[10px] text-muted-foreground", className)}
    {...props}
  />
));
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Separator
    ...
    className={cn("mx-1 my-1 h-px bg-[color:color-mix(in_oklab,var(--foreground),transparent_90%)]", className)}
    {...props}
  />
));
