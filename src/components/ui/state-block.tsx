...
export function StateBlock({
  ...
  return (
    <Alert
      ...
      className={cn(
        "relative overflow-hidden rounded-[1.4rem] border px-4 py-4 shadow-[inset_0_1px_0_color-mix(in_oklab,var(--card),white_30%),var(--shadow-soft)]",
        tone === "neutral" && "border-border bg-[color:color-mix(in_oklab,var(--accent),transparent_84%)] text-foreground",
        tone === "error" &&
          "border-[color:color-mix(in_oklab,var(--destructive),transparent_70%)] bg-[color:color-mix(in_oklab,var(--destructive),transparent_88%)] text-destructive",
        tone === "warning" &&
      ...
      <div
        ...
        className={cn(
          ...
          tone === "error" && "bg-[radial-gradient(circle_at_0%_50%,color-mix(in_oklab,var(--destructive),transparent_86%),transparent_72%)]",
      ...
      <div className="relative min-w-0">
        ...
        {description ? (
          <AlertDescription className="mt-1 text-xs leading-relaxed opacity-90">
            <p>{description}</p>
          </AlertDescription>
        ) : null}
      </div>
